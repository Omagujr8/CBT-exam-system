...
def create_exam(db: Session, creator_id: int, exam_in: ExamCreate):
    ...
    # Assign an access token if provided or generate a random one
    try:
        token = getattr(exam_in, "access_token", None)
    except Exception:
        token = None
    if token:
        exam.access_token = token
    else:
        # generate a short url-safe token
        exam.access_token = secrets.token_urlsafe(8)
    db.add(exam)
    db.commit()
    db.refresh(exam)
    return exam
...
def get_exam_token(db: Session, exam_id: int):
    exam = db.query(Exam).filter(Exam.id == exam_id).first()
    if not exam:
        return None
    return {"access_token": exam.access_token}


def regenerate_exam_token(db: Session, exam_id: int):
    exam = db.query(Exam).filter(Exam.id == exam_id).first()
    if not exam:
        return None
    exam.access_token = secrets.token_urlsafe(8)
...
def get_exam_by_token(db: Session, token: str):
    if not token:
        return None
    return db.query(Exam).filter(Exam.access_token == token).first()
