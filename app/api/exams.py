...
# --- Token endpoints for exam access ---
@router.get("/{exam_id}/token")
def get_exam_token(
    exam_id: int,
    db: Session = Depends(get_db),
    ...
    if getattr(current_user, "role", None) == "teacher":
        if not exam_service.teacher_can_access_exam(db, current_user.id, exam):
            raise HTTPException(status_code=403, detail="Not allowed to access token for this exam")
    token = exam_service.get_exam_token(db, exam_id)
    if token is None:
        raise HTTPException(status_code=404, detail="Token not found")
    return token


@router.post("/{exam_id}/regenerate-token")
def regenerate_exam_token(
    exam_id: int,
    db: Session = Depends(get_db),
    current_user = Depends(require_role(["teacher", "admin"]))
):
    ...
    if getattr(current_user, "role", None) == "teacher":
        if not exam_service.teacher_can_access_exam(db, current_user.id, exam):
            raise HTTPException(status_code=403, detail="Not allowed to regenerate token for this exam")
    token = exam_service.regenerate_exam_token(db, exam_id)
    if token is None:
        raise HTTPException(status_code=500, detail="Failed to generate token")
    return token


@router.get("/by-token/{token}", response_model=ExamOut)
def get_exam_by_token(token: str, db: Session = Depends(get_db)):
    exam = exam_service.get_exam_by_token(db, token)
    if not exam:
        raise HTTPException(status_code=404, detail="Exam not found for provided token")
    return exam
